...
using VOA.CouncilTax.AutoProcessing.BulkProcessor.Functions.Routing;
using VOA.CouncilTax.AutoProcessing.BulkProcessor.Functions.Services;
...
public sealed class BulkDataRequestProcessor
{
        ...
        string? processingRunId = null)
    {
        ...
        var createResult = await service.CreateBatchAsync(
            createItems.Select(item => item.Payload),
            submitUserId,
            componentName,
            sourceType,
            jobTypeId: jobTypeId,
        ...
        {
            if (!outcomeBySsu.TryGetValue(item.Payload.SsuId, out var outcome))
            {
                continue;
            }
    ...
    /// <summary>
    /// Calls the custom API 'voa_UpsertHereditamentLinkv1' to validate and resolve SSU ID in PostgreSQL.
    /// </summary>
    private async Task<(bool Success, Guid ResolvedSsuId, string ErrorMessage)> CallUpsertHereditamentLinkApiAsync(Guid ssuId)
    {
        try
        {
            ...
            if (response != null && response.Results.TryGetValue("HereditamentIdentifiers", out var result))
            {
                // The response contains SSU ID; parse it
                var ssuResponse = JsonSerializer.Deserialize<StatutorySpatialUnitResponse>(
                    response.Results["HereditamentIdentifiers"].ToString() ?? string.Empty);

                if (ssuResponse != null && Guid.TryParse(ssuResponse.StatutorySpatialUnitId, out var resolvedId))
                {
                    _logger.LogInformation("Upsert hereditament succeeded. SsuId={ResolvedId}", resolvedId);
                    return (true, resolvedId, string.Empty);
                }
            }

            _logger.LogWarning("Upsert hereditament returned invalid response for SsuId={SsuId}", ssuId);
            return (false, Guid.Empty, "Invalid response from api");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Upsert hereditament API call failed for SsuId={SsuId}", ssuId);
            return (false, Guid.Empty, ex.Message);
        }
    }

    /// <summary>
    /// Response model for the voa_UpsertHereditamentLinkv1 custom API.
    /// </summary>
    private sealed class StatutorySpatialUnitResponse
    {
        [System.Text.Json.Serialization.JsonPropertyName("statutorySpatialUnitId")]
