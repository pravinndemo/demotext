...
There is no automated test project in this repository. The chunk logic lives in small, pure helper methods that can be validated in isolation:

- **`GetChunkSizeFlag` / `GetIntConfigValue`** — pure static functions; test by setting environment variables and asserting return values for valid, missing, unparseable, and out-of-range inputs.
